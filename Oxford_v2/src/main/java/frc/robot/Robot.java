...
public class Robot extends TimedRobot {
  ...
  //private RobotContainer m_robotContainer;
    ...
    //low wheels
    WPI_TalonFX LW = new WPI_TalonFX(10);
    //ArmMotor Motor
    WPI_TalonFX ArmMotor = new WPI_TalonFX(8);
    //Joysicks
    ...
    //Limit Switches
    AnalogInput MainArmLimit = new AnalogInput(0);
    AnalogInput LoserArmLimit = new AnalogInput(1);
  ...
  case Right_April:
    ...
    if(fl == 2 && Target == 1){
      if(area > 0.3){
        mecanum.driveCartesian(-0.5, 0.2, 0);
      }else{
        fl = 3;
      }
    }else if(fl == 3 && Alex.getAngle() < 90){
  ...
  public void ScoreHighCube(){
    double shc = 0;
    if(MainArmLimit.getVoltage() < 0 && shc == 0){
      ArmMotor.set(0.4);
    }else{
      ArmMotor.set(0);
      shc = 1;
    ...
    }else if(LoserArmLimit.getVoltage() < 0){
      ArmMotor.set(-0.4);
    }
    else{
      GrabberIn.set(false);
